<template>
    <v-card class=" login-card" elevation="8">
        ...
        <v-card-text class="login-form">
            <v-text-field v-model="username" :label="$t('login.username')" autocomplete="username" dense name="username"
                outlined persistent-placeholder class="input-field"></v-text-field>
        ...
        <v-card-actions class="login-actions">
            <v-btn @click="submit()" color="primary" block class="login-btn">
                {{ $t("login.loginButton") }}
            </v-btn>
